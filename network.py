...
class Network:
    def __init__(self, X, y):
        ...
        self.train()
    def variables(self):
        self.x = tf.placeholder(tf.float64, shape=(None, self.train_X.shape[1]*self.train_X.shape[2]*self.train_X.shape[3]))
        self.y = tf.placeholder(tf.bool, shape=(None, self.train_Y.shape[1]*self.train_Y.shape[2]))
        self.w = tf.Variable(xavier_init(self.train_X.shape[1]*self.train_X.shape[2]*self.train_X.shape[3],self.train_Y.shape[1]*self.train_Y.shape[2]), name="weight")
        self.b = tf.Variable(tf.zeros([self.train_Y.shape[1]*self.train_Y.shape[2]]), dtype = tf.float64)
