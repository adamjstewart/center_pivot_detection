...
class CNNTS3D_add(nn.Module):
    def __init__(self, args, loadable_state_dict=None, inC=6, inT=10, embedding_size=16):
        ...
        # Temporal Conv
        self.temporal_embedder = nn.Sequential(  # T = 10
            nn.Conv2d(inC, 16, (3, 1), stride=(1, 1), padding=(1, 0)),  # T = 10
            nn.LeakyReLU(),
            nn.BatchNorm2d(16),
            nn.Conv2d(16, 32, (4, 1), stride=(2, 1), padding=(1, 0)),  # T = 5
            nn.LeakyReLU(),
            nn.BatchNorm2d(32),
            nn.Conv2d(32, embedding_size, (4, 1), stride=(2, 1), padding=(1, 0)),  # T = 2
            nn.LeakyReLU(),
        )
        # Spatial
    ...
    def forward(self, x):
        ...
        x = x.view(N, self.embedding_size * 2, 1, H, W).view(N, -1, H, W)
...
class CNNTS2D_add(nn.Module):
    def __init__(self, args, loadable_state_dict=None, inC=6, inT=10):
